    ...
    if ($row > 0) {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
          ...
          VALUES ('$id_fact','$id_cl', '$id_typ', '$arrive', '$depart', '$chb_nb', '$pers_nb', '$prix_resa')";
                ...
                header('Location: infocl.php?login_err=password');
                die();
            }
        } else {
            header('Location: infocl.php?login_err=email');
            die();
        }
    } else {
        header('Location: infocl.php?login_err=already');
        die();
    }
} else {
    header('Location: infocl.php');
    die();
