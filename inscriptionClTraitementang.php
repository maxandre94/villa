            ...
            if (strlen($pren) <= 100) {
                //if(preg_match("#[0][1]|[0][5]|[0][7][- \.?]?([0-9][0-9][- \.?]?){4}$#", $tel)){
      	    	  ...
      	    	  VALUES ('$langue', '$civilite', '$nom', '$pren', '$email', '$password', '$tel', '$token')";
                                                    ...
                                                    header('Location: inscriptionClientang.php?reg_err=tel');
                                                    die();
                                                }
                                            } else {
                                                header('Location: inscriptionClientang.php?reg_err=pay_length');
                                                die();
                                            }
                                        } else {
                                            header('Location: inscriptionClientang.php?reg_err=cp_length');
                                            die();
                                        }
                                    } else {
                                        header('Location: inscriptionClientang.php?reg_err=ville_length');
                                        die();
                                    }
                                } else {
                                    header('Location: inscriptionClientang.php?reg_err=etat_length');
                                    die();
                                }
                            } else {
                ...
                header('Location: inscriptionClientang.php?reg_err=pren_length');
                die();
            }
        } else {
            header('Location: inscriptionClientang.php?reg_err=nom_length');
            die();
        }
    } else {
        header('Location:inscriptionClientang.php?reg_err=already');
        die();
    }//il existe déjà
}
