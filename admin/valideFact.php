    ...
    if ($client['langue'] == 'Français') {
        ...
        'Messages' => [
            [
                ...
                'HTMLPart' => '<h3>Hello '.$client['civilite_cl'].' '.mb_strtoupper($client['nom_cl']).' your reservation n° '.$fact['id_fact'].' of '.date('d/m/Y H:i:s', strtotime($fact['date_fact'])).' amounting to '.number_format($fact['montant'], 0, ',', ' ').' XOF has been confirmed. Please proceed to payment via your client interface on the site or click on the link http://localhost:8888/villa/reservation/payConnectCl.php?token='.$client['token'].'.</h3><br />Thank you and have a nice stay at villa blanca.',
                'CustomID' => 'AppGettingStartedTest',
            ],
        ],
    ];
