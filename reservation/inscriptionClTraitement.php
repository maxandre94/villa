                                                ...
                                                if (preg_match('#[0-9]$#', $tel)) {
                                                    if (filter_var($email, FILTER_VALIDATE_EMAIL)) { // Si l'email est de la bonne forme
                                                        if ($password === $password_retype) {
                                                            ...
                                                            $query = $bdd->prepare("INSERT INTO client (adress, pays, etat, ville, code_pos, langue, civilite_cl, nom_cl, prenom_cl, email, mot_pas, tel_cl, adress, pays, etat, ville, code_pos, token)
      	    	  VALUES ('$adress', '$code_pay', '$etat', '$ville', '$cp', '$langue', '$civilite', '$nom', '$pren', '$email', '$password', '$tel', '$adress', '$code_pay', '$etat', '$ville', '$cp', '$token')");
